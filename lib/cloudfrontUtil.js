...
/**
 * Build AWS signed cookies.
 *
 * @param {String} CloudFront resource URL
 * @param {Object} Signature parameters
 * @return {Object} Signed Cookies
 */
function getSignedCookies(cfUrl, params) {
  // If an expiration time isn't set, default to 30 seconds.
  var defaultExpireTime = Math.round(new Date().getTime() + 30000);
  var expireTime = _getExpireTime(params) || defaultExpireTime;
  var policy = params.customPolicy || new CannedPolicy(cfUrl, expireTime);
  var policyJSON = policy.toJSON();
  var sign = crypto.createSign('RSA-SHA1');
  var cookies = {};
  var privateKeyString = params.privateKeyString;
  var pem;
  ...
  if (params.customPolicy) {
    cookies['CloudFront-Policy'] = normalizeSignature(
      new Buffer(policyJSON).toString('base64')
    );
  }
  else {
    cookies['CloudFront-Expires'] = policy.expireTime.toString();
  }
  ...
  signature = sign.sign(privateKeyString, 'base64');
  cookies['CloudFront-Signature'] = normalizeSignature(signature);
...
exports.CustomPolicy = require('./CustomPolicy');
